...
\citet{Gabriel:2014} resolved this ambiguity by demonstrating that, within local mixing length theory, the neutrality criterion must be applied on the convective side to ensure that convective velocities vanish at the boundary. However, they were unable to provide a concrete approach to implementing this behavior in codes that rely on diffusive, rather than instantanous, mixing. In \citet{Paxton:2017}, co-PI Townsend developed a new \emph{predictive mixing} scheme in the MESA code; this scheme adaptively modifies the convective mixing diffusivites to 
